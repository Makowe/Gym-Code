...
import 'package:gym_code/classes/rulesets/ruleset_parallel_bars.dart';
import 'package:test/test.dart';

import 'constants/test_routines_parallel_bars.dart';

var ruleSet = ParallelBarsRuleSet();

void main() {
  group('Parallel Bars Ruleset inherits general Ruleset behavior', () {
    ...
    test('Routine validity, valuing and scoring match the general ruleset',
      ...
      var evaluatedByParallelBars = mediumRoutineWithDismount.copy();

      generalRuleSet.evaluateRoutine(evaluatedByGeneral);
      ruleSet.evaluateRoutine(evaluatedByParallelBars);

      expect(evaluatedByParallelBars.elements.map((e) => e.isValued),
          evaluatedByGeneral.elements.map((e) => e.isValued));
      expect(evaluatedByParallelBars.result?.dScore,
          evaluatedByGeneral.result?.dScore);
      expect(evaluatedByParallelBars.result?.groups,
          evaluatedByGeneral.result?.groups);
      expect(evaluatedByParallelBars.result?.penalty,
